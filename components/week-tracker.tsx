...
export default function WeekTracker({
  ...
  const handleCreateWeek = async () => {
    ...
    if (result.error) {
      ...
      setSuccessMessage("New week started!");
  ...
  const handleLockWeek = async () => {
    ...
    if (
      confirm(
        "Are you sure you want to finish this week? You won't be able to edit it anymore."
      )
    ) {
      ...
      if (result.error) {
        ...
        setSuccessMessage("Week completed!");
      }
    }
  };
  ...
  const handleAddExercise = async () => {
    ...
    const result = await addAdHocExercise(
      planId,
      activeDayId,
      newExerciseName,
      newExerciseSets,
    ...
    if (result.error) {
      ...
      setSuccessMessage("Exercise added to plan!");
      setNewExerciseName("");
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-card/50 backdrop-blur-sm p-4 rounded-xl border border-white/5 shadow-sm gap-4">
        <div className="flex items-center gap-4 w-full md:w-auto">
          <Select value={selectedWeekId} onValueChange={setSelectedWeekId}>
            <SelectTrigger className="w-full md:w-60 bg-black/20 border-white/10 text-white">
            ...
            <SelectContent className="bg-[#1a2332] border-white/10 text-white">
              {weeks.map((week) => (
                ...
                >
                  Week of {new Date(week.start_date).toLocaleDateString()}{" "}
                  {week.is_locked ? "(Completed)" : "(Active)"}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          {!weeks.length && (
            <span className="text-sm text-gray-400">No active weeks</span>
          )}
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <Dialog>
            ...
            <DialogContent className="bg-[#1e293b] border-white/10 text-white sm:max-w-106.25">
              <DialogHeader>
                <DialogTitle>Start New Week</DialogTitle>
                <DialogDescription className="text-gray-400">
                  Select the start date for this week. You can backfill past
                  weeks.
                </DialogDescription>
              </DialogHeader>
              <div className="py-6">
          ...
          {currentWeek && !currentWeek.is_locked && (
            <Button
              ...
              className="flex-1 md:flex-none border-red-500/50 text-red-400 hover:bg-red-500/10 hover:text-red-300"
            >
          ...
          <TabsList className="bg-black/20 p-1 rounded-xl mb-6 flex flex-wrap h-auto gap-1">
            {planDays.map((day) => (
              <TabsTrigger
                ...
                className="flex-1 min-w-25 data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-lg rounded-lg transition-all"
              >
                Day {day.day_order}
              </TabsTrigger>
            ))}
                ...
                <CardContent className="space-y-6 pt-6">
                  {day.exercises.map((exercise) => {
                    ...
                    return (
                      <div
                        key={exercise.id}
                        className="p-6 rounded-xl bg-black/20 border border-white/5 hover:border-white/10 transition-colors"
                      >
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                          ...
                          {!isLocked && (
                            <Button
                              size="sm"
                              onClick={() =>
                                handleSaveLog(exercise.id, day.day_order)
                              }
                              disabled={isSaving}
                            ...
                            >
                              {["Easy", "Medium", "Difficult"].map((diff) => (
                                ...
                                >
                                  ...
                                  />
                                  <Label
                                    htmlFor={`${diff}-${exercise.id}`}
                                    className="font-normal text-gray-300 cursor-pointer hover:text-white transition-colors"
                                  >
                                    {diff}
                                  </Label>
                                </div>
