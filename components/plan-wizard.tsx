...
export default function PlanWizard({
  ...
  const [daysPerWeek, setDaysPerWeek] = useState<string>(
    initialPlan?.days_per_week.toString() || ""
  );
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < days.length; i++) {
      if (!days[i].headline) {
        setFormError(`Please add a headline for Day ${i + 1}`);
        return;
      }
      if (days[i].exercises.length === 0) {
        setFormError(`Please add at least one exercise for Day ${i + 1}`);
        return;
      }
      for (const ex of days[i].exercises) {
        if (!ex.name) {
          setFormError(
            `Please provide a name for all exercises in Day ${i + 1}`
          );
          return;
        }
      }
    ...
    <div className="max-w-3xl mx-auto py-10 px-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {step === 1
              ? "Start by giving your plan a name."
            ...
            <div className="space-y-6">
              {isEditing && (
                <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-md text-sm text-yellow-800">
                  Note: Reducing days or removing exercises that have logs might
                  cause display issues in history.
                </div>
              )}
